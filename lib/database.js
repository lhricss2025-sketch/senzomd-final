 ...
 * this removes the read-modify-write races that used to lose data.
 *
 * Safety features:
 *  - Atomic writes (temp file + rename) — a crash mid-write can never corrupt
 *    an existing file.
 *  - Corrupted files are backed up (file.broken-<ts>) instead of killing reads.
 *  - Cross-process pair lock (O_EXCL lockfile) — a second bot instance cannot
 *    request a second pairing code at the same time as the first one.
 */
const fs = require("fs");
...
function premiumList() {
  return Object.entries(read("premium"))
...
function getReferrals(userId) {
  ...
  const r = refs[userId] || {};
  return { link: r.link || null, joined: (r.joined || []).filter(Boolean), unlocked: r.unlocked || false };
}
...
// ── Boost: connected member numbers ──
...
// ── Pair TOKENS (one-shot /pair → /code flow) ──
...
// ── Cross-process pairing lock ──
// Guarantees that only ONE bot process requests a pairing code at a time —
// a second instance (e.g. a duplicate Railway deploy) will see the lock and
// skip instead of generating a second, different code.
const PAIR_LOCK_PATH = path.join(DB_DIR, ".pairing.lock");
const PAIR_LOCK_STALE_MS = 60000;

function acquirePairLock() {
  try {
    const fd = fs.openSync(PAIR_LOCK_PATH, "wx");
    fs.writeFileSync(fd, String(process.pid));
    try { fs.closeSync(fd); } catch {}
    return true;
  } catch (err) {
    if (err.code !== "EEXIST") return true; // FS hiccup — don't block pairing
    // Stale lock from a crashed process → steal it
    try {
      const st = fs.statSync(PAIR_LOCK_PATH);
      if (Date.now() - st.mtimeMs > PAIR_LOCK_STALE_MS) {
        try { fs.unlinkSync(PAIR_LOCK_PATH); } catch {}
        return acquirePairLock();
      }
    } catch {}
    return false;
  }
...
module.exports = {
  ...
  acquirePairLock, releasePairLock,
